...
%% ff parameters
config.ff.parameters.vx = -0.003:0.00002:0.003;
config.ff.parameters.vy = -0.003:0.00002:0.003;
config.ff.parameters.dl = 0:0.0012:0.006;


config.ff.v.x = @(vx,dl) vx;
config.ff.v.y = @(vy) vy;
config.ff.v.z = @(vx,vy,dl) sqrt(1 - vx.^2 - vy.^2);

config.ff.l.x = @(dl) dl;
config.ff.l.y = @() 0;
config.ff.l.z = @(dl) sqrt(1 -(dl).^2);
