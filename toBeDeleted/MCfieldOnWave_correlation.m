function [C,Cs]=MCfieldOnWave_correlation(af_ang_vl,Wl,dirl,dirv,sigt,albedo,box_min,box_max,l,v,is_ff_l,is_ff_v,maxItr,lambda,doCBS,smpFlg,sct_type,ampfunc,ampfunc0,lmean0)
...
%     w0p = 1;
    ...
    while 1
        ...
        %add path to field
        C = C + tpath(1:Nv/2,1).*conj(tpath((Nv/2+1):end,2));
        
        if (pL==1)
            Cs = Cs + tpath(1:Nv/2,1).*conj(tpath((Nv/2+1):end,2));
        end
...
C=C*(1/maxItr*V*sigt(2));
Cs=Cs*(1/maxItr*V*sigt(2));
