function [u,umean]=MCfieldFourierQuadNF(sigt,albedo,box_min,box_max,l,v_max,v_stp,maxItr,lambda,doCBS,smpFlg,lmean0)
...
xa=tx1L(1,:)./tx1L(3,:);
ya=tx1L(2,:)./tx1L(3,:);
ii=find((abs(xa)<tan(v_max)).*(abs(ya)<tan(v_max)));
expRmax=1/maxItr*sum(exp(-sigt*x1L(3,ii))./max(abs(x1L(3,ii)-l(3)),1).^2)*Nv0^4/(4*pi)

expRmax=1/maxItr*sum(exp(-sigt*x1L(3,ii))./max(abs(x1L(3,ii)-l(3)),1/(2*v_max)).^2)*Nv0^4/(4*pi)


expRmean=1/maxItr*sum(exp(-sigt*x1L(3,ii)))*Nv0^4/(4*pi)
    ...
    for j1=1:Nz-1
       ...
       %zscl=exp(-2*pi*i/lambda*tz*vz0-sigt/2*((tz-box_min(end)).*diratt0));
        parfor j2=1:Nl
            ...
            %wl=exp((2*pi*i/lambda-sigt/2)*dl)./(max(dl,lambda*2));
            wl=exp((2*pi*i/lambda*dl));
            for j3=1:length(jj)
...
u=u*sqrt(1/maxItr)/(sqrt(pi)*2);%*V*sigt);

umean=zeros(size(u));
for j2=1:Nl
    umean(:,:,j2)=exp(-sigt/2*(l(3,j2)-box_min(end)).*diratt0).*exp(-2*pi*i/lambda*(l(3,j2)*vz0+ l(2,j2)*vy0+ l(1,j2)*vx0));
    
end
umean=umean/(sqrt(pi)*2);
xL;
%x1L
