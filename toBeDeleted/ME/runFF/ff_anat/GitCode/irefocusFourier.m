...
Nx_targ=Nv_targ;%1/v_stp/2/box_stp(1)*2+1;
...
u=reshape(u,Nx,Nx);
u=padarray(u,[Nx_padd_size,Nx_padd_size]);
...
for j=1:Nz
    ...
    tu=u;
    %size(tu)
    %size(tu(1+Nx_padd_size:end-Nx_padd_size,1+Nx_padd_size:end-Nx_padd_size))
    tu=flipud(fliplr(fftshift(fft2(ifftshift(tu)))));
    ...
    rU(:,:,j)=elz*tu;%flipud(fliplr(fftshift(fft2(ifftshift(tu)))));
...
rU=rU(Nv_padd_size+1:end-Nv_padd_size,Nv_padd_size+1:end-Nv_padd_size,:);
return
size(elxy)
for j=1:Nz
   rU(:,:,j)=rU(:,:,j).*elxy;
