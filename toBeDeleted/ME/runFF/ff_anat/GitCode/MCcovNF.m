function [Ms,Mm]=MCcovNF(sigt,albedo,box_min,box_max,l,v,is_ff_l,is_ff_v,maxItr,lambda,doCBS,smpFlg,sct_type,ampfunc,ampfunc0,lsign,vsign)
...
for itr=1:maxItr
    ...
    switch smpFlg
        ...
        case 3
            ...
            [x,px]=radialExpSmpX(box_min,box_max,l,sigt,lambda,lsign);
    end
    ...
    if ~is_ff_v
      ...
      rv=rv.*repmat(vsign,dim,1);
    end
    if ~is_ff_l
      rl=repmat(x,1,Nl)-l;
      rl=rl./repmat(sum(rl.^2,1).^0.5,dim,1);
      rl=rl.*repmat(lsign,dim,1);
    end
    ...
    e_l0=evalphaseatt(x,l,is_ff_l,sigt,lambda,box_min,box_max,lsign);
    ...
    % and the sensors
    e_v0=evalphaseatt(x,ff_sign*v,is_ff_v,sigt,lambda,box_min,box_max,vsign);
    ...
    while 1
        ...
        if (pL>1)
            ...
            e_v=evalphaseatt(x,ff_sign*v,is_ff_v,sigt,lambda,box_min,box_max,vsign);
            af_v=evalampfunc_general((ow'*rv),sct_type,ampfunc,dim);
            e_v_ms=e_v.*af_v;

            if doCBS

                e_l=evalphaseatt(x,l,is_ff_l,sigt,lambda,box_min,box_max,lsign);
