function [u,us]=MCfieldGPU(sigt,albedo,box_min,box_max,l,v,is_ff_l,is_ff_v,maxItr,lambda,doCBS,smpFlg,sct_type,ampfunc,ampfunc0,lmean0)
...
u = gpuArray(zeros(Nv,Nl));
if nargout>1
    us = gpuArray(zeros(Nv,Nl));
end
...
af_ang_vl = gpuArray(zeros(Nv, Nl));
if (is_ff_v && is_ff_l)
    for j=1:Nl
        af_ang_vl(:,j)=evalampfunc_generalGPU(l(:,j)'*v,sct_type,ampfunc,dim);
    end
end
   ...
   %itr
    ...
    switch smpFlg
        case 1
            ...
            %x=[itr/(maxItr+1);0.5;0.8].*box_w+box_min
            %x=[1/(maxItr+1);0.5;0.01].*box_w+box_min;
        case 2
    ...
    if ~(is_ff_v && is_ff_l)
        for j=1:Nl
            af_ang_vl(:,j)=evalampfunc_generalGPU(rl(:,j)'*rv,sct_type,ampfunc,dim);
        end
    end
    ...
    % function in article).
    af_l=evalampfunc_generalGPU((w'*rl),sct_type,ampfunc,dim)./w0p;
    ...
    e_l0=gpuArray(evalphaseatt(x,l,is_ff_l,sigt,lambda,box_min,box_max));
    ...
    % and the sensors
    e_v0=gpuArray(evalphaseatt(x,ff_sign*v,is_ff_v,sigt,lambda,box_min,box_max));
    ...
    % first scatter
    if doCBS
         af_v=evalampfunc_generalGPU((-w'*rv),sct_type,ampfunc,dim)./w0p;
         e_v0_ms=e_v0.*af_v;
    end
    ...
    while 1
        ...
        if (pL>1)
            %break;
            e_v=gpuArray(evalphaseatt(x,ff_sign*v,is_ff_v,sigt,lambda,box_min,box_max));
            af_v=evalampfunc_generalGPU((ow'*rv),sct_type,ampfunc,dim);
            e_v_ms=e_v.*af_v;
            if doCBS
                e_l=gpuArray(evalphaseatt(x,l,is_ff_l,sigt,lambda,box_min,box_max));
                af_l=evalampfunc_generalGPU((-ow'*rl),sct_type,ampfunc,dim);
                e_l_ms=e_l.*af_l;
            end
        end
