function [u,umean,FI]=MCfieldFourierQuadNF(sigt,albedo,box_min,box_max,l,v_max,v_stp,maxItr,lambda,doCBS,smpFlg,lmean0)
...
for itr=1:maxItr
    ...
    switch smpFlg
        case 1
            ...
            x=rand(dim,1).*(box_w)+box_min; px=1/V;
        case 2
        ...
        case 3
            ...
            x=l(:,ind)+d*w; 
            px=1;   
            dl=sqrt(sum(l.^2)-2*x'*l+sum(x.^2,1));
            wl=sigt*exp((-sigt)*dl)./(max(dl,lambda*0.1)).^2;
            px=1/Nl*sum(wl)*1/(4*pi);
    end
    if( max(max(x>box_max),max(x<box_min)))
         continue
     end
    ...
    for j1=1:Nz-1
       ...
       %zscl=exp(-2*pi*i/lambda*tz*vz0-sigt/2*((tz-box_min(end)).*diratt0));
        for j2=1:Nl
            ...
            u_xy=zeros(Nx,Nx);
            dl=sqrt(sum(l(:,j2).^2)-2*l(:,j2)'*xL(:,jj)+sum(xL(:,jj).^2,1));
            wl=sqrt(sigt)/(2*sqrt(pi))*exp((2*pi*i/lambda-sigt/2)*dl)./(max(dl,lambda*0.1));
            for j3=1:length(jj)
               ...
               u_xy(inds(j3))=u_xy(inds(j3))+wl(j3)*pxL(1,jj(j3)).*exp(-2*pi*i/lambda*(-x1L(3,jj(j3))+tz));%.*exp(-sigt/2*((xL(end,jj(j3))-box_min(end)).*l_diratt+(box_max(end)-xL(end,jj(j3))).*l_idiratt));
...
%% Normalization
%V=prod(box_w);
u=u*sqrt(1/maxItr)/(sqrt(pi)*2);
umean=zeros(size(u));
for j2=1:Nl
    umean(:,:,j2)=exp(-sigt/2*(l(3,j2)-box_min(end)).*diratt0).*exp(-2*pi*i/lambda*(l(3,j2)*vz0+ l(2,j2)*vy0+ l(1,j2)*vx0))/(sqrt(pi)*2);
...
if 1
    w0=lambda*5;
    for j2=1:Nl
         dl=sqrt(sum(l(:,j2).^2)-2*l(:,j2)'*xL+sum(xL.^2,1));
         wl=sqrt(sigt)/(2*sqrt(pi))*exp((-sigt/2)*dl)./(max(dl,lambda*0.2));
         wl1=exp(-sigt/2*(x1L(3,:)-box_min(3)))/(2*sqrt(pi));
         w=wl.*pxL.*wl1;
         
         wl0=exp(-sigt/2*(l(3,:)-box_min(3)))/(2*sqrt(pi))
         
         [tgx,tgy]=meshgrid([-20:20]*lambda,[-20:20]*lambda);
         
         ctr_L=[tgx(:)'+l(1,j2);tgy(:)'+l(2,j2);l(3,j2)*ones(size(tgx(:)'))];
         for j1=1:size(ctr_L,2)
           e=evalGaussianBeamGrid(ctr_L(:,j1),w0,[0;0;1],lambda,x1L);
           e0=evalGaussianBeamGrid(ctr_L(:,j1),w0,[0;0;1],lambda,l(:,j2));
           tE(j1)=1/maxItr*sum(abs(e.*w).^2);
           tE0(j1)=abs(e0*wl0)^2;
         end
         FI(:,:,j2)=reshape(tE,size(tgx,1),size(tgx,2));
         FI(:,:,j2+Nl)=reshape(tE0,size(tgx,1),size(tgx,2));
    end
