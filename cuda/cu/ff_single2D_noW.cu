...
__global__ void ff_single2D_noW(double2* us, double2* e_l0, double2* af_ang_vl,
    const double* v1, const double* v2, const double* dir_v1, const double* dir_v2)
{
    ...
    if (uIdx < (vSize * lSize))
    {
        ...
        double* x, * constCont;
        double2 vlMult, tpath, attRotation, currAf_ang_vl;
        ...
        // multiple with const
        tpath.x = fma(vlMult.x, attRotation.x, -vlMult.y * attRotation.y);
        tpath.y = fma(vlMult.x, attRotation.y, vlMult.y * attRotation.x);
