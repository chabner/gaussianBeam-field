...
__global__ void integrateMult(double2* u, double2* e_l0_ms,
    ...
    const double* dir_v1, const double* dir_v2, const double* dir_v3)
{
    ...
    if (uIdx < (vSize * lSize))
    {
        ...
        if (fw == 1)
        {
            af_v = sqrt(g_up / pow(fma(g_down2, cosang, g_down1), 1.5));
        }
        else
        {
            af_v = sqrt(fw * g_up / pow(fma(g_down2, cosang, g_down1), 1.5) +
