...
__global__ void integrateMultMultiplePaths(double2* u, const double* dirv, const double* x, const double2* randomPhase,
    ...
    const double2* vApertureMu1, const double2* vApertureMu2, const double2* vApertureMu3, const double2* vApertureC)
{
    ...
    if (uIdx < uDimProd[3])
    {
        ...
        for (int mixtureIdx = 0; mixtureIdx < lDim[0]; mixtureIdx++)
        {
            ...
            complexIncrease(u + uIdx, (lMixtureAlpha[mixtureIdx] * randomPhase[uDimIdx[3]]) * (complexExponent(lMixtureC[currlIdx] + vThroughputC + sqrtMu) / sqrtMu));
