...
__global__ void ff_single_noW(double2* us, double2* e_l0, double* af_ang_vl,
    ...
    const double* dir_v1, const double* dir_v2, const double* dir_v3)
{
    ...
    if (uIdx < (vSize * lSize))
    {
        ...
        // multiple with const
        tpath.x = fma(vlMult.x, constCont[0], -vlMult.y * constCont[1]);
        tpath.y = fma(vlMult.x, constCont[1], vlMult.y * constCont[0]);
